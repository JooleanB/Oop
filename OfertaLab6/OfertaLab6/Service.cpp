...
void Agentie::modificaOferta(const string& denumire, const string& destinatie, const string& denumire_noua, const string& destinatie_noua, const string& tip_nou, int pret_nou)
{
	Oferta p = rep.find(denumire, destinatie);
	string tip_vechi = p.getTip();
	int pret_vechi = p.getPret();
	rep.modify(denumire,destinatie, denumire_noua, destinatie_noua, tip_nou, pret_nou);
	Oferta p2 = rep.find(denumire_noua, destinatie_noua);
	this->undoList.push_back(std::make_unique<UndoModifica>(UndoModifica{ p2,rep,denumire,destinatie,tip_vechi,pret_vechi }));
	stergeStatistica(destinatie);
